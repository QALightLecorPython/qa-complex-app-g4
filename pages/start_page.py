...
class StartPage(BasePage):
    ...
    def sign_in(self, user: User) -> HelloUserPage:
        ...
        self.fill_field(
            xpath=self.constants.SIGN_IN_USERNAME_XPATH, value=user.username
        )
        self.fill_field(
            xpath=self.constants.SIGN_IN_PASSWORD_XPATH, value=user.password
        )
    ...
    def verify_sign_in_error(self) -> None:
        ...
        error_message = self.wait_until_displayed(
            xpath=self.constants.SIGN_IN_ERROR_MESSAGE_XPATH
        )
        assert (
                error_message.text == self.constants.SIGN_IN_ERROR_MESSAGE_TEXT
        ), "Text is not valid"
    ...
    def sign_up_and_verify(self, user: User) -> HelloUserPage:
        ...
        self.fill_field(
            xpath=self.constants.SIGN_UP_USERNAME_XPATH, value=user.username
        )
        self.fill_field(
            xpath=self.constants.SIGN_UP_EMAIL_XPATH, value=user.email
        )
        self.fill_field(
            xpath=self.constants.SIGN_UP_PASSWORD_XPATH, value=user.password
        )
    ...
    def click_sign_up_and_verify(self) -> None:
        ...
        self.click(xpath=self.constants.SIGN_UP_BUTTON_XPATH)
        assert not self.is_element_exists(
            xpath=self.constants.SIGN_UP_BUTTON_XPATH
        )
    ...
    def verify_sign_up_validation_error(self, text):
        ...
        validation_errors = self.driver.find_elements(
            by=By.XPATH, value=self.constants.SIGN_UP_FIELD_VALIDATION_MESSAGE
        )
        visible_errors = [
            element for element in validation_errors if element.is_displayed()
        ]
        assert len(visible_errors) == 1, f"Actual: {len(visible_errors)}"
        assert (
                visible_errors[0].text == text
        ), f"Actual: {visible_errors[0].text}"

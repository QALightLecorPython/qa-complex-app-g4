...
class Chat(BasePage):
    ...
    def send_message(self, message: str) -> None:
        ...
        self.fill_field(
            xpath=self.constants.CHAT_INPUT_FIELD_XPATH,
            value=message + Keys.ENTER,
        )
    ...
    def verify_messages(self, expected_messages: List[str]):
        ...
        elements = self.wait_until_elements_displayed(
            self.constants.CHAT_MESSAGES_XPATH
        )
        actual_messages = [element.text for element in elements]
        assert (
                actual_messages == expected_messages
        ), f"Actual: {actual_messages}, Expected: {expected_messages}"

...
class ProfilePage(BasePage):
    ...
    def navigate_to_the_post(self, title):
        ...
        posts = self.driver.find_elements(by=By.XPATH, value=self.constants.POSTS_LIST_XPATH)
        for post in posts:
            if post.text == title:
                post.click()
    ...
    def verify_deleted_message_text(self) -> None:
        ...
        element = self.wait_until_displayed(self.constants.POST_DELETED_MESSAGE_XPATH)
    ...
    def verify_posts_count(self, expected_count: int):
        ...
        assert tab_posts_count == expected_count, f"Actual: {tab_posts_count}, Expected: {expected_count}"

        # Verify actual posts list
        posts = self.driver.find_elements(by=By.XPATH, value=self.constants.POSTS_LIST_XPATH)
        assert len(posts) == expected_count, f"Actual: {len(posts)}, Expected: {expected_count}"

...
class HelloUserPage(BasePage):
    ...
    def __init__(self, driver):
        ...
        self.chat = Chat(driver)

    @log_wrapper
    def verify_success_sign_up(self, username):
        """Verify hello message for a new user"""
        hello_message = self.wait_until_displayed(
            xpath=self.constants.SUCCESS_MESSAGE_XPATH
        )
        assert username.lower() in hello_message.text
        assert hello_message.text == self.constants.SUCCESS_MESSAGE_TEXT.format(
            username=username.lower()
        )
        assert (
                self.wait_until_displayed(
                    xpath=self.constants.SUCCESS_MESSAGE_USERNAME_XPATH
                ).text
                == username.lower()
        )
